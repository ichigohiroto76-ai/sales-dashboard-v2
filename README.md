...
技術構成は HTML / CSS / JavaScript のみとし、Cloudflare Pages にそのままデプロイできる静的サイトとして構築します。
...
| salesMethods | array | 営業方法一覧 |
| priorities | array | 優先度一覧 |
| temperatures | array | 温度感一覧 |
...
| salesMethod | 任意 | Store.salesMethod | 電話 / メール / LINE / Instagram / 訪問 / 紹介 / その他 |
| priority | 任意 | Store.priority | 高 / 中 / 低 |
| temperature | 任意 | Store.temperature | 高 / 中 / 低 / 不明 |
...
| phone | 任意 | Store.phone | 電話番号 |
| email | 任意 | Store.email | メールアドレス |
| address | 任意 | Store.address | 住所 |
| mapUrl | 任意 | Store.mapUrl | GoogleマップURL |
| lineUrl | 任意 | Store.lineUrl | LINE URL |
| instagramUrl | 任意 | Store.instagramUrl | Instagram URL |
| leadSource | 任意 | Store.leadSource | 流入元 |
| probability | 任意 | Store.probability | 0から100の数値 |
| lostReason | 任意 | Store.lostReason | 見送り理由 |
...
```csv
id,name,groupName,areaName,ownerName,status,salesMethod,priority,temperature,lastContactDate,nextContactDate,appointmentAt,nextAction,memo,phone,email,address,mapUrl,lineUrl,instagramUrl,leadSource,probability,lostReason,createdAt,updatedAt
```
...
静的サイトとして運用するため、初期実装ではブラウザの LocalStorage にデータを保存します。
...
Googleスプレッドシート同期を追加する場合でも、画面側は LocalStorage を直接参照せず、サービス層を通じてデータを取得します。

これにより、将来的に保存先が LocalStorage から外部APIに変わっても、画面側の変更を最小限に抑えます。
...
- Googleスプレッドシート同期
  - LocalStorage の代替またはバックアップ先として利用する
...
- 画面は LocalStorage を直接操作せず、サービス層を通じてデータを扱う
...
本プロジェクトは HTML / CSS / JavaScript のみで構成された静的サイトです。
...
注意: 現在のデータ保存先はブラウザの LocalStorage です。そのため、同じURLで開けますが、登録データは端末・ブラウザごとに保存されます。複数端末で同じデータを共有するには、将来的に Googleスプレッドシート同期などの外部保存機能を追加します。
...
- GitHubへ push するファイルは、`index.html`、`README.md`、`assets/`、`sample-data/` を含める
- 外部API連携を追加する場合は、認証情報をフロントエンドへ直接埋め込まない
- Cloudflare Pages Functions を使う場合は、別途設計を更新する
